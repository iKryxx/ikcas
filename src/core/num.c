...
rat_t rat_from_i64(int64_t num) { return (rat_t){num, 1}; }

bool rat_is_zero(rat_t rat) { return rat.num == 0; }
...
void rat_to_str(rat_t rat, char *buf, int cap) {
    rat = rat_norm(rat);
    if (rat.den == 1)
        snprintf(buf, (size_t)cap, "%lld", (long long)rat.num);
    else
        snprintf(buf, (size_t)cap, "%lld/%lld", (long long)rat.num,
                 (long long)rat.den);
}
