...
static bool check_arity(int min, int max, int got, const char* name, const char** err) {
    if (min > max) return false;
    if (min <= got && max >= got) return true;
    if (!err) return false;
    if (min == max) {
        sprintf(*(char**)err, "%s takes %d argument.", name, min);
    }
    else {
        sprintf(*(char**)err, "%s takes between %d and %d arguments.", name, min, max);
    }
    return false;
}

// ---------- Abs: take the absolute value of node. 1 Arg ----------
node_t * fn_abs_cb(arena_t* a, node_t** args, int argc, lookup_fn_t lookup, void* lctx, const char** err) {
    unused(lookup);
    unused(lctx);
