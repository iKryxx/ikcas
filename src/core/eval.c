...
static eval_result_t eval_symbol_impl(arena_t* a, const node_t* n, lookup_fn_t lookup, void* lctx);
static eval_result_t eval_call_impl(arena_t* a, const node_t* n, lookup_fn_t lookup, void* lctx);
static eval_result_t resolve_callable_impl(const char* name, lookup_fn_t lookup, void* lctx, const node_t** out_callable);
static eval_result_t eval_binop_impl(arena_t* a, node_kind_t kind, const node_t* l, const node_t* r, lookup_fn_t lookup, void* lctx);
static eval_result_t approx_impl(arena_t* a, const node_t* n, lookup_fn_t lookup, void* lctx);

static eval_result_t ok(node_t *n) {return (eval_result_t) {.ok=true, .err = nullptr, .out = n};}
static eval_result_t fail(const char* err) {return (eval_result_t) {.ok=false, .err = err, .out = nullptr};}

static bool is_rat(const node_t* n) {return n && n->kind == NODE_RAT;}
static bool is_zero_rat(const node_t* n) {return is_rat(n) && rat_is_zero(rat_norm(n->rat));}

static bool is_one_rat(const node_t* n) {return is_rat(n) && rat_norm(n->rat).num == rat_norm(n->rat).den;}
...
static bool approx_const_lookup(const char* name, double* out) {
    for (size_t i = 0; i < sizeof(APPROX_CONSTS) / sizeof(APPROX_CONSTS[0]); i++) {
        if (strcmp(APPROX_CONSTS[i].name, name) == 0) {
            if (out) *out = APPROX_CONSTS[i].value;
            return true;
        }
    }
    return false;
...
static eval_result_t eval_symbol_impl(arena_t *a, const node_t *n, lookup_fn_t lookup, void *lctx) {
    if (!n || n->kind != NODE_SYMBOL) return fail("not a symbol");
    ...
    while (cur->kind == NODE_SYMBOL) {
        ...
        for (int i = 0; i < sp; i++) {
            if (strcmp(name, seen[i]) == 0) return fail("cyclic definition");
        }
        if (sp >= 64) return fail("definitions too deep");
...
static eval_result_t eval_call_impl(arena_t *a, const node_t *n, lookup_fn_t lookup, void *lctx) {
    ...
    eval_result_t rc = resolve_callable_impl(n->call.name, lookup, lctx, &callee);
    if (!rc.ok) return rc;
    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (!ei.ok) return ei;
        args[i] = ei.out;
        if (!args[i]) return fail("null arg");
    }
...
static eval_result_t resolve_callable_impl(const char *name, lookup_fn_t lookup, void *lctx, const node_t **out_callable) {
    ...
    for (;;) {
        for (int i = 0; i < sp; i++) {
            if (strcmp(seen[i], cur) == 0) return fail("cyclic definition");
        }
        if (sp >= 64) return fail("definitions too deep");
        seen[sp++] = cur;
...
static eval_result_t eval_binop_impl(arena_t *a, node_kind_t kind, const node_t *l, const node_t *r, lookup_fn_t lookup, void *lctx) {
    ...
    switch (kind) {
        case NODE_ADD:
            if(is_zero_rat(l_)) return ok(r_);
            if(is_zero_rat(r_)) return ok(l_);
            if(is_rat(l_) && is_rat(r_)) return ok(mk_rat(a, rat_add(l_->rat, r_->rat)));
            return ok(mk_bin(a, NODE_ADD, l_, r_));
        case NODE_SUB:
            if(is_zero_rat(r_)) return ok(l_);
            if(is_rat(l_) && is_rat(r_)) return ok(mk_rat(a, rat_sub(l_->rat, r_->rat)));
            if(is_zero_rat(l_)) return ok(mk_neg(a, r_));
            return ok(mk_bin(a, NODE_SUB, l_, r_));
        case NODE_MUL:
            if (is_zero_rat(l_) || is_zero_rat(r_)) return ok(mk_rat(a, rat_from_i64(0)));
            if (is_one_rat(l_)) return ok(r_);
            if (is_one_rat(r_)) return ok(l_);
            if (is_rat(l_) && is_rat(r_)) return ok(mk_rat(a, rat_mul(l_->rat, r_->rat)));
            if (!is_rat(l_) && is_rat(r_)) return ok(mk_bin(a, NODE_MUL, r_, l_));
        ...
        case NODE_DIV:
            ...
            if (is_rat(l_) && is_rat(r_)) {
                ...
                if (!okd) return fail("division by zero");
        ...
        case NODE_POW:
            if(is_rat(l_) && is_rat(r_)) {
                ...
                if (e.den == 1) {
                    ...
                    if (!okp) return fail("invalid power");
...
static node_t* mk_real_bin(arena_t* a, node_kind_t kind, double lhs, double rhs, const char** err) {
    switch (kind) {
        case NODE_ADD: return node_real(a, lhs + rhs);
        case NODE_SUB: return node_real(a, lhs - rhs);
        case NODE_MUL: return node_real(a, lhs * rhs);
        case NODE_DIV:
            if (rhs == 0.0) {
                if (err) *err = "division by zero";
                return nullptr;
            }
...
static eval_result_t approx_impl(arena_t* a, const node_t* n, lookup_fn_t lookup, void* lctx) {
    ...
    switch (n->kind) {
        ...
        case NODE_SYMBOL: {
            double v = 0.0;
            if (approx_const_lookup(n->symbol, &v)) return ok(node_real(a, v));
            return ok(node_symbol(a, n->symbol, -1));
        }
        case NODE_NEG: {
            eval_result_t x = approx_impl(a, n->unary.a, lookup, lctx);
            if (!x.ok) return x;
            if (is_real(x.out)) return ok(node_real(a, -x.out->real));
        ...
        case NODE_POW: {
            ...
            if (is_real(l.out) && is_real(r.out)) {
                ...
                node_t* out = mk_real_bin(a, n->kind, l.out->real, r.out->real, &err);
                if (!out) return fail(err ? err : "approximation failed");
                return ok(out);
            }
        ...
        case NODE_CALL: {
            ...
            if (lookup) {
                ...
                const callable_t* c = callee->callable;
                const int argc = n->call.argc;
                if (argc < c->min_arity || argc > c->max_arity) return fail("wrong arity");
                ...
                if (c->kind == CALL_BUILTIN) {
                    ...
                    node_t* folded = bf->eval_exact ? bf->eval_exact(a, args, argc, lookup, lctx, &berr) : nullptr;
                    if (berr) return fail(berr);
                    if (folded) return approx_impl(a, folded, lookup, lctx);
            ...
            return ok(node_call(a, arena_strdup(a, n->call.name, -1), args, n->call.argc));
        }
        case NODE_CALLABLE:
            return ok((node_t*)n);
        default:
